...
export function GenRandomQ10(){
    ...
    const question_text = "ความเร็วขนาด u เมตร/วินาที มีค่าเท่าไหร่ในหน่วย กิโลเมตร/ชั่วโมง".split(" ")
    ...
    while (choice.length < choiceAmount) {
        ...
        let randomOffset = getRandomIntMN(1, 10); // Random number between 1 and 5
        let WrongChoice = choice[getRandomIntMN(0, choice.length - 1)] + (randomOffset * ((-1) ** getRandomIntMN(0, 1)));
        
        while (WrongChoice <= 0) {
            randomOffset = getRandomIntMN(1, 10); // Regenerate random offset if necessary
